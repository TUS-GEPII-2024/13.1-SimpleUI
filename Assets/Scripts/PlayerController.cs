...
public class PlayerController : MonoBehaviour
{
    ...
    // How much to multiple the characters normal x velocity by when in a dash
    public float dashMultiplier = 1.0f;

    // A boolean to determine whether or not the Character has the ability to dash. This might
    // be useful as it allows for a level design that only allows the character to dash when
    // certain conditions are true. The LevelManager will be able to set this to true or false
    // as desired.
    public bool dashEnabled;

    // How long, in seconds, a dash lasts for
    ...
    // A variable to store whether the "dash button" has been pressed (currently the
    // Q key)
    private bool doDash;

    // A boolean to indicate whether or not the character is in a dash. We need this as
    // we don't want the charcter to "double dash" (dash in a dash)
    ...
    void Update()
    {
        // Only read the spacebar of jump is false because if it is true this means we
        // have read the spacebar and detected it was pressed and if we read it again
        // we will likely detect that it is no longer pressed. So jump could go from
        // false -> true ->  false before our code in FixedUpdate has executed and seen
        // that it is true.
        if (jump == false)
        {
    ...
    void FixedUpdate()
    {
        ...
        else if (grounded && jump)
        {
            ...
            // Now that we have read the jump variable as true, reset it to false so as to allow the 
            // spacebar to be read again in the Update function
            jump = false;
        }
